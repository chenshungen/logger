...
func (s *severity) Set(value string) error {
	...
	if v, ok := severityByName(value); ok {
		...
		return errors.New("unknow name log level")
	}
	logging.fileLogThreshold.set(threshold)
	logging.stderrThreshold.set(threshold)
	return nil
}

func fileLogThresholdSet(value string) error {
	var threshold severity
	// Is it a known name?
	if v, ok := severityByName(value); ok {
		threshold = v
...
func stderrThresholdSet(value string) error {
...
func init() {
	...
	logging.fileLogThreshold = debugLog
...
type loggingT struct {
	...
	// Level flag. Handled atomically.
	fileLogThreshold severity // The -stderrthreshold flag.
...
func (l *loggingT) formatHeader(s severity, file string, line int) *buffer {
	...
	// 新版本格式 2021/05/21 by chet
	// buf.WriteByte(severityChar[s])
	// _, month, day := now.Date()
	// buf.WriteString(fmt.Sprintf("%02d%02d ", month, day))
	buf.WriteString(now.Format("2006-01-02 15:04:05"))
	buf.WriteString(" ")
	buf.WriteString(severityName2[s])
	buf.WriteString(" ")
	buf.WriteString(processIdxStr)
	buf.WriteString(" ")
...
// Write parses the standard logging line and passes its framework to the
...
func (l *loggingT) checkLogLevel(s severity) bool {
	if s < l.fileLogThreshold.get() {
		return false
	}
	return true
}
...
func Debug(args ...interface{}) {
	if !logging.checkLogLevel(debugLog) {
		return
	}
...
func DebugfDepth(depth int, format string, args ...interface{}) {
	...
	logging.printfDepth(debugLog, depth, format, args...)
}

// Info logs to the INFO log.
// Arguments are handled in the manner of fmt.Print; a newline is appended if missing.
func Info(args ...interface{}) {
	if !logging.checkLogLevel(infoLog) {
		return
	}
...
func Warning(args ...interface{}) {
	if !logging.checkLogLevel(warningLog) {
		return
	}
...
func Error(args ...interface{}) {
	if !logging.checkLogLevel(errorLog) {
		return
	}
...
func FatalDepth(depth int, args ...interface{}) {
	if !logging.checkLogLevel(fatalLog) {
		return
	}
